...
export async function blingPost(path, payload) {
  return blingFetch(path, { method: 'POST', body: payload ? JSON.stringify(payload) : undefined });
}
...
export async function criarPedido(order, items) {
  ...
  let numero = data.numero ?? null;
  if (data.id && numero == null) {
    // o POST não devolve o número — busca logo em seguida (visto no 1º envio real, pedido 2867)
    const det = await blingFetch(`/pedidos/vendas/${data.id}`);
    numero = det.body?.data?.numero ?? null;
  }
  return { blingId: data.id ?? null, numero, payload, blingBody: body };
}
